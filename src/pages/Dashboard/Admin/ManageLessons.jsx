...
function ManageLessons() {
    ...
    const limit = 6; // Lessons per page
    ...
    // Data Fetching: Now includes 'page' and 'limit' in params
    const { data: lessonData = {}, isLoading, refetch: allLessonsRefetch, error } = useQuery({
        queryKey: ["adminLessons", filters, page], // <-- ADDED: 'page' as dependency
        queryFn: async () => {
            const params = {
                ...filters,
            ...
            const res = await axiosSecure.get(`/admin/all-lessons`, { params }); 
            return res.data;
        },
    });
    ...
    const totalLessons = lessonData?.totalLessons || 0; // for better display
    ...
    const handleFilterChange = (e) => {
        setPage(1); // <-- IMPORTANT: Reset page to 1 when filters change
        setFilters(prev => ({ 
            ...prev, 
    ...
    const handleDeleteLesson = async (lesson) => {
        ...
        if (result.isConfirmed) {
            await axiosSecure.delete(`/lessons/${lesson._id}`); // Check your backend URL here
            Swal.fire({
                title: "Deleted!",
    ...
    return (
        <div className="w-full py-10 px-4">
            ...
            <div className=" mb-8 p-4 bg-gray-50 rounded-xl shadow-inner border border-gray-200">
                <h3 className="font-semibold text-lg w-full mb-3 text-gray-700">Filter Lessons ({totalLessons} found)</h3>
                <div className="flex flex-col md:flex-row justify-between gap-4">
                    {/* Selects... (Code is same as yours) */}
                    <select 
                        name="category"
                ...
                <table className="table table-zebra min-w-[1100px] w-full">
                    ...
                    <tbody>
                        {allLessons.map((lesson, index) => (
                            <tr key={lesson._id}>
                                <th className="whitespace-nowrap">{(page - 1) * limit + index + 1}</th> {/* S.I. adjusted for page */}

                                {/* Lesson Info (Code is same as yours) */}
            ...
            {totalPages > 1 && (
                <div className="flex justify-center mt-6 gap-2">
                    ...
                    <span className="px-4 py-2 font-semibold border rounded-lg bg-white">
